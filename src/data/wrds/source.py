...
class WRDSSource(DataSource):
    ...
    def get_estimates(self, tickers: Sequence[str], start: date, end: date) -> list[EstimateData]:
        """Pending: ``ibes.det_epsus`` (34.5M individual analyst estimates → TrueBeats)."""
        raise NotImplementedError("IBES estimates land in the next step (#55)")

    def get_fundamentals(
        self, tickers: Sequence[str], start: date, end: date
    ) -> list[FundamentalData]:
        """Pending: ``comp.fundq`` filtered by ``tic``, point-in-time via ``rdq``."""
        raise NotImplementedError("Compustat fundamentals land in the next step (#55)")
